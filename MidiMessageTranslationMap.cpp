...
void MidiMessageTranslationMap::Translate( MidiMessage * msg ) {
    ...
    // databyte3 translations
    if ( msgT->second.GetDataByte3MapSize() > 0 ) {
        std::pair < uint8_t, uint8_t > tp = msgT->second.GetDataByte3MapPairByValue( msg->GetDataByte3() );
        if ( tp.first != 0 && tp.second != 0 ) {
            uint8_t newDataByte3 = tp.second;
            msg->SetDataByte3( newDataByte3 );
        }
    }
